...
contract ArbitrageVault is Ownable, ReentrancyGuard {
    ...
    ) external nonReentrant onlyBridgeProcessor returns (uint256) {
    ...
    // Withdraw profits
    function withdrawProfits(uint256 amount) external onlyBotOperator nonReentrant {
    ...
    // Update bridge processor
    function updateBridgeProcessor(address newProcessor) external onlyOwner {
        require(newProcessor != address(0), "Invalid address");
        bridgeProcessor = newProcessor;
    }
    
    // Update bot operator
    function updateBotOperator(address newOperator) external onlyOwner {
        require(newOperator != address(0), "Invalid address");
        botOperator = newOperator;
    }
